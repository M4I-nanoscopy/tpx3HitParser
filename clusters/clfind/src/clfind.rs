...
const TIME_WINDOW: i64 = 50;

pub fn clfind(hits: ArrayView2<i64>) -> ndarray::Array1<i64> {
    ...
    while i < hits.len_of(Axis(0)) {
        ...
        if labels[i] == -1 {
            ...
            clfind_sub(i, hit, cluster_index, &mut s, &mut labels, hits);
...
fn clfind_sub(
    ...
    ls: &mut ndarray::Array1<i64>, hs: ndarray::ArrayView2<i64>,
) {
    ...
    for i in *start_idx..hs.len_of(Axis(0)) {
        ...
        if ((x0 - x1).abs() <= 1) && ((y0 - y1).abs() <= 1) {
            let tmp_start_idx = clfind_sub(j, h1, c_id, start_idx, ls, hs);
        }
    }
}
